...
# ══════════════════════════════════════════════════════════════════════
# DATA JS FILE (for HTML dashboard)
# ══════════════════════════════════════════════════════════════════════

DATA_JS_FILE = SCRIPT_DIR / "macro_data.js"

def write_data_js(data_dict):
    """
    Write a JavaScript file that the HTML dashboard loads via <script> tag.
    This avoids CORS issues since <script src> is not subject to CORS.
    """
    js_data = {
        "fedFundsRate": data_dict.get("DFF", data_dict.get("FEDFUNDS")),
        "treasury10Y": data_dict.get("DGS10"),
        "treasury2Y": data_dict.get("DGS2"),
        "tips10Y": data_dict.get("DFII10"),
        "breakeven10Y": data_dict.get("T10YIE"),
        "cpiYoY": data_dict.get("CPIAUCSL"),
        "corePCE": data_dict.get("PCEPILFE"),
        "cpiMoM": data_dict.get("CPIAUCSL_MOM"),
        "forward5Y5Y": data_dict.get("T5YIFR"),
        "hySpread": data_dict.get("BAMLH0A0HYM2"),
        "igSpread": data_dict.get("BAMLC0A0CM"),
        "aaSpread": data_dict.get("BAMLC0A3CA"),
        "tedSpread": data_dict.get("TEDRATE"),
        "unemployment": data_dict.get("UNRATE"),
        "nfp": data_dict.get("PAYEMS"),
        "initialClaims": data_dict.get("ICSA"),
        "jolts": data_dict.get("JTSJOL"),
        "gdpGrowth": data_dict.get("GDP"),
        "vix": data_dict.get("VIXCLS"),
        "sp500": data_dict.get("SP500"),
        "lastUpdated": datetime.now().strftime("%Y-%m-%d %H:%M"),
    }
    # Remove None values
    js_data = {k: v for k, v in js_data.items() if v is not None}

    try:
        content = "// Auto-generated by macro_data_updater.py\n"
        content += f"// Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        content += f"var FRED_DATA = {json.dumps(js_data, indent=2)};\n"
        with open(DATA_JS_FILE, "w", encoding="utf-8") as f:
            f.write(content)
        log.info(f"Data JS file written: {DATA_JS_FILE}")
    except OSError as e:
        log.warning(f"Could not write data JS file: {e}")
...
def main():
    ...
    # ── Always write macro_data.js for HTML dashboard ─────────────
    write_data_js(transformed)
